...
CONDITION_NE_REGEX = re.compile(r'\s* ([:\w"]+) \s* !~ \s* \/ (.+) \/ \s* $', re.S | re.X)
...
def parseCondition(s):
    ...
    if CONDITION_NE_REGEX.match(s):
        a = CONDITION_NE_REGEX.match(s).groups()
        log.debug("condition NE_REGEX: %s = %s" % (a[0], a[1]))
        return Condition('ne_regex', a)
